\documentclass[12pt, a4paper]{report}
    ...
    \newglossaryentry{Rolling shutter}
    {
      ...
      description={is reading out pixels as a row at a time from the sensor rather than capturing all pixel values at one when the camera needs to capture an image \cite[ch. 6]{ReadTheDocsPicamera}}
    }
  ...
  \newglossaryentry{Time scale}
    ...
    {
      name=SAD,
      description={(sum of absolute differences) is a positive number, which results from the formation of the difference between two digital images. It serves as a measure of the difference between two images and is used in image processing and pattern recognition. It is obtained by subtracting the color values of the images pixel by pixel from each other and adding them up by amount \cite{Richardson2003}}
    }
    
    \newglossaryentry{MMAL}
    {
      name=MMAL,
      description={(Multimedia Abstraction Layer) is a framework designed by Broadcom to provide a host-side, simple and relatively low-level interface to multimedia components running on the Videocore IV GPU on the Raspberry Pi \cite{videocore}}
    }
    ...
    {
      name=Codecvisa,
      description={is a powerful real-time analyzer for different video codecs \cite{codecian}}
    }
  ...
  \newglossaryentry{API}
    {
      name=API,
      description={(Application Programming Interface) is a clear defined interface to communicate between components}
    }
   ...
   \newglossaryentry{Photon}
    ...
    {
      name=Motion blur,
      description={is the apparent streaking of rapidly moving objects in a still image or a sequence of images such as a movie or animation}
    }

    \newglossaryentry{Raspberry Pi}
    {
      name=Raspberry Pi,
      description={is a credit-card-sized single-board computer for hobby and education use}
    }
    ...
    {
      name=VideoCore IV,
      description={is a multimedia processor low powered and optimized for image applications such as image compression and decompression \cite{videocore}}
    }
    ...
    { \huge \bfseries Measurement of Rate Deviation}\\[0.4cm] % Title of your document
      { \huge \bfseries  of Mechanical Watches -}\\[0.4cm] 
      { \huge \bfseries  an Optical Approach using}\\[0.4cm]
       { \huge \bfseries  Raspberry Pi}\\[0.4cm]% Title of your document
...
Mechanical watches must be checked for rate deviations both during manufacture and use. Most of the equipment used for this purpose is expensive. Almost all of the devices on the market apply acoustic measurements and a few complement this measurement with an optical measurement done with a laser beam. In this the question has been tackled whether a Raspberry Pi and a Raspberry Pi camera can be turned into a cheaper and simple measuring device that can compete with the accuracy of a professional device using a new approach: image processing. 
\newline
...
\newline
In a first step, possible methods were evaluated in order to carry out the optical measurement of the frequency of the balance wheel of a watch. A closer look was taken at tracking a specific pixel, the OpenCV, the Gstreamer and the Picamera library. The Picamera library was used eventually since it offers a very detailed documentation and many of the functionalities needed for this work. In order to better understand the motion vectors, various experiments were carried out, whereby the camera settings were also improved. Using the data and knowledge from these experiments, an implementation of a Python program was carried out. The experiments also showed that the rate deviation of the watch reacts very sensitively to varying temperatures, making it difficult to get reliable reference measurements. For this reason, a setup using a blinking LED with known frequency was chosen as a reference. The obtained results showed that the determination of the frequency by means of image processing worked very well and that sufficiently precise results could be achieved. In order to get better direct reference measurements, some measurings of a watch were done in parallel with the camera and a Greiner measurement device. It turned out that the results of both devices are very similar and therefore the measurement with the camera is accurate enough. For a precision of 5, 3, and about 2s one needs approximately 40s, 150s and 600s of measurement accordingly. In order to turn the presented approach into a product which can compete with the professional devices, some more implementations of measurements of various other things need to be made. 
...
\pagebreak
    ...
    Today's watch technology requires measuring instruments to perform a wide variety of measurements and analyses on mechanical watches with great precision and reliability.
    They must be checked in production and service for their rate deviation.
    ...
    The second chapter will give a background for this work. Acoustical and optical measurement will be explained and compared. One will have a look at the functionality and operation of mechanical watches, the used camera module and its constraints as well as the Nyquist–Shannon sampling theorem.


    In the third chapter different algorithms as optical flow, kalman filtering, object tracking and motion vectors, and libraries for image processing as OpenCV, Gstreamer and Picamera library will be discussed and evaluated. 
    ...
    The last chapter summarizes those results and outlines the problems that occured during the work. It also gives an outlook of what could be improved and extended to get a device which can compete with professional ones.

    \chapter{Background}
    This chapter is intended to lay the foundation for this report. An overview of the existing measuring methods for the accuracy of a mechanical watch is given and the Raspberry Pi camera, which was used for the optical measurements in this project, is described. In addition, the sample theorem and the functionality of a mechanical watch is explained and important information about the test watch is listed.
    ...
    About hundred years later, at the end of the 20th century, optical watch timing machines joined acoustical ones. Even nowadays there are only a few devices on the market which employ only optical measurement, but several companies started to combine acoustical with optical measurements. One way to determine the frequency of the balance wheel optically is by using a laser. The beam is periodically interrupted by the balance wheel and thus the frequency can be calculated \cite{Lombardi2011}. 
    
    In this paper the frequency is quantified optically by using image processing. There is no known professional device which uses this technique for measuring the frequency.
    ...
    \subsubsection{Witschi - Watch Expert II}
    Watch Expert II tests mechanical watches acoustically and optically. The measurement is done in parallel and is more accurate as two signals are used for the calculation of the frequency.
    The optical measuring is done using a laser and lighting. A camera helps to adjust the watch properly.  This device is also able to measure the amplitude, which corresponds to the angle of rest position of the balance wheel until turning point. The price of their product is: CHF 10450.- .
    ...
    The Compact 900 measures the beat noises of the lever escapement. Just like the device from Witschi, it is also able to measure the amplitude. The price is CHF 4070.- .
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{ |c||L|L|L|  }
     ...
     \hline
     &{\fontsize{9}{10}\selectfont \textbf{Witschi - Watch Expert II}}  & {\fontsize{8}{9}\selectfont \textbf{Lepsi - WatchScope/WatchAnalyzer} }& {\fontsize{9}{10}\selectfont \textbf{Greiner Vibrograf - Compact 900} }\\\hline
 ...
 \end{table}
    ...
    This keeps the readout time of each line constant. However, the CPU does not "speak" directly to the camera, but the processing rather takes place on the GPU (VideoCore IV) of the Raspberry Pi which operates its own real-time operating system (VCOS). Doing so the load of the CPU is reduced.    
    \bigskip
    \noindent
    \begin{figure}[H]
    \centering
    ...
    \end{itemize}
      ...
      In order to convert a continuous-time signal, such as a movement into a discrete-time signal, such as a time stamp, the Nyquist–Shannon sampling theorem must be respected in order to ensure that the continuous-time signal is correctly reconstructed.
      The sampling theorem states if a signal contains no frequencies higher than a specific number of hertz ($B$), it is completely determined by giving its ordinates at a series of points spaced $1/(2B)$ seconds apart. A sufficient sample rate is therefore $2B$ samples/second \cite{Seibt2006}.
      In this project the continous-time component is represented by the balance wheel (i.e. 3 Hz) and the discrete-time component is represented by the frame rate and its time stamps of the camera (90 Hz) which satisfies the condition for correct measurements.  
      \pagebreak
      
    \section{Mechanical Watches}  
    In the following, the function of a mechanical watch is explained in more detail and the role of a watch's balance wheel is explained. Subsequently, important information on mechanical watches, such as the duration of strokes and facts about the test watch used in this project, are described.
...
In the following section, the movement of the balance wheel, which will be recorded by the camera and used for the calculation of the frequency, is explained.
...
\bigskip
    ...
    n\text{*} = 2f*3600
     ...
     \pagebreak
    The following table lists important facts about mechanical wristwatches. Among other things, it contains the number of strokes per hour and the duration for one stroke, i. e. half a period.
    ...
    \end{tabularx}
       \caption{  Number of strokes, period duration and frequency of the balance wheel of automatic wristwatches \cite{Krug1987}}
        \end{table}

        \subsection{Test Watch}
      The test watch, which was used, is of the caliber ETA C01.211. It is a chronograph with a diameter of 31 centimeters. It strokes 21600 times per hour with a stroke duration of 0.166666 seconds. It therefore has a frequency of 3 Hz. Additionally, the watch has a power reserve of 43 hours \cite{Caliber}.
...
The implementation in Open CV is based on the Gunner Farneback's algorithm which is explained in "Two-Frame Motion Estimation Based on Polynomial Expansion" by Gunner Farneback in 2003 \cite{Farneback2003}.
 ...
 Motion vectors of the MMAL (Multi-Media Abstraction Layer) are used in video compression to reduce resources required to store and transmit data. A motion vector describes the displacement of a pixel or pixel block within an image sequence \cite{Motionvector}. 
 The motion vectors can be used to describe how much change is taking place in an image sequence. This can be used to identify a moving or still balance wheel. 
   ...
   \begin{table}[H]
      ...
      \centering
       ...
       \textbf{}&   {\fontsize{8}{10}\selectfont Generaliz-ability }&  {\fontsize{8}{10}\selectfont Computa-tional Intensity} &   \multicolumn{3}{c|}{\fontsize{8}{10}\selectfont Implemented and Easy Accessible} \\ \hline
        ...
        {\fontsize{8}{10}\selectfont Object Tracking}     & \ding{51}        & high   & \ding{51}     & \ding{55}    & \ding{55}\\ \hline
        {\fontsize{8}{10}\selectfont Motion \newline Vectors and SAD of MMAL} & \ding{51}  & very low  & \ding{55}   & \ding{55} & \ding{51}\\ \hline
      \end{tabularx}
    ...
    With the Picamera library it is easy to access these motion data values so the hard part was not data access but the understanding of how they are constructed and which camera settings return the best data. 
   ...
   The test setup is the same as shown in Figure 6.3, but without the Greiner Compact 900 and its accessories. On average, the camera was positioned approximately four centimeters from the test watch. In order to ensure that the images are still well illuminated and a good measurement can be carried out due to the high shutter speed, a small and strong light source directed at the test watch was used. The test watch was positioned on a turn table so it was easy to adjust the position of the watch.
 ...
 As this worked quite well, the balance wheel of the test watch was recorded. Unfortunately, this program can only detect very clear movements. With very small movements this program fails, therefore the frequency of the balance wheel could not be calculated with this implementation. 
   ...
   Another approach was to animate all SAD values, which are a measure of the difference between two images, with colors to determine whether they can be used to calculate the frequency of the movement of the balance wheel. In Figure 4.2 a snapshot of the animation of the SAD values is shown. The lighter the color of a pixel, the higher is the value of the SAD. 
    ...
    This experiment is based on the idea of the optical flux with the motion vectors as displayed in Figure 4.6 .
...
During the experimental sessions with the motion vectors it became clear how the information contained in the motion vectors could be used to calculate the oscillation of the balance wheel. At first, a crude calculation was made in Excel and the gained insights were then used to start implementing a program in Python. The following chapter will explain how those calculations and implementations were done.
    ...
    In order to roughly calculate the frequency, an Excel sheet was created - Figure 5.1 is a snippet - in which all the accumulated absolute x- and y-values were entered and the standstills of the balance wheel were determined by eye. The half-oscillations and the frequency were calculated from the time intervals of the standstills. Afterwards, the average of all calculated frequencies was determined and it was recognized that the results with this method are a good approach to determine the frequency and thus the rate deviation. 
    ...
    \caption{Illustration of aggregated x-(blue), y-values (red) over time. The x-axis is the time and the y-axis are the x- and y-values.}
    \end{figure}
...
where $n_2$ ist the number of strokes per hour, which is 21600.
    ...
    The Figure below and Table 6.1 show the deviation per day based on the preeceding calculation.
    ...
    Knowing that the methods do not only work in theory but also in practice, a measurement of a Greiner Compact 900 and the Raspberry Pi camera was executed in parallel. In Figure 6.3 the setup is shown. The watch was placed on a nimble mounting of the Greiner device, which includes the microphone, that was used for measuring the oscillation of the balance wheel. 
    
      \begin{figure}[H]
        \centering
        \includegraphics[scale=0.24]{Images/camera_and_microphone}
   ...
   From the approach of the regression
     ...
     and the values of the duration of measurement (t in s.) as well as the metered absolute measurement errors (y in s.) the regression equation for the measurement error looks as follows
...
A^T y = 
\begin{pmatrix}
\frac{1}{t_1}*y_1 +\frac{1}{t_2}*y_2 + \ldots + \frac{1}{t_n}*y_n \\ y_1 + y_2 + \ldots + y_n \end{pmatrix}
 \end{equation}
...
The plot on top in Figure 7.1 are the measured values and the plot below in the same figure is drawn with the computed function. As one can easily see the graph of the calculated function matches really good with the timeline of the measured values, which proves that the error truly is proportional to 1 over time. 
      ...
      \begin{figure}[H]
        \centering
        \includegraphics[scale=0.4]{Images/error_measurement_100_600}
